...
def _ignore_url_build_changes(base_dir):
    print(f"fixing build url only changes for path '{base_dir}'...")
    line_slugs = [
        "-  - url:",
        "-    sha256:",
        "-  # - url:",
...
def _is_changed_or_not_tracked(pth):
    ctracked = subprocess.run(
        f"git ls-files --error-unmatch {pth}",
        shell=True,
        capture_output=True,
    )
    ...
    for dist, arch in dist_arch_tuples:
        ...
        for cdt, cfg in cdts.items():
            ...
            if "build_number_bump" in cfg:
                _extra += f" --build-number-bump={cfg['build_number_bump']}"
            ...
            if os.path.exists(pth):
                try:
                    ...
                    print(
                        ...
                        f"recipe for custom CDT {cdt}: {e!r}"
                    )
...
def _clear_gen_cdts(pth):
    try:
        ...
        print(f"WARNING: error removing autogenerated CDTs at {pth}: {e!r}")


def _fix_cdt_licenses(*, cdts, dist_arch_tuples, cdt_path):
    print(f"fixing CDT licenses for path '{cdt_path}'...", flush=True)
    for dist, arch in dist_arch_tuples:
        for cdt, cfg in cdts.items():
            pth = os.path.join(
...
def _fix_cdt_deps(*, cdts, dist_arch_tuples, cdt_path):
    print(f"adjusting CDT deps for path '{cdt_path}'...", flush=True)
    for dist, arch in dist_arch_tuples:
        for cdt, cfg in cdts.items():
            pth = os.path.join(
                cdt_path,
...
def _fix_cdt_builds(*, cdts, dist_arch_tuples, cdt_path):
    print(f"adjusting CDT builds for path '{cdt_path}'...", flush=True)
    for dist, arch in dist_arch_tuples:
        ...
        for cdt, cfg in cdts.items():
            ...
            if (
                ...
                if distarch in cfg["build_append"]:
                    ...
                    raise RuntimeError(f"could not get build append for {cdt}!")
